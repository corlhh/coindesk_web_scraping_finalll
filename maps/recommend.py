...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_x = mean(xs)
    mean_y = mean(ys)  
    x_diff = [x - mean_x for x in xs]
    y_diff = [y - mean_y for y in ys]
    xy_pair = zip(x_diff, y_diff)
    Sxx = sum([x**2 for x in x_diff])
    Syy = sum([y**2 for y in y_diff])
    Sxy = sum([x * y for x, y in xy_pair])
    b = Sxy / Sxx
    a = mean_y - b * mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor_and_r = [find_predictor(user, restaurants, feature_fn) for feature_fn in feature_fns]
    #max_predictor_and_r = max(predictor_and_r, key=lambda x: x[1])
    return max(predictor_and_r, key=lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    return {restaurant_name(restaurant): predictor(restaurant) for restaurant in restaurants}
    # END Question 9
